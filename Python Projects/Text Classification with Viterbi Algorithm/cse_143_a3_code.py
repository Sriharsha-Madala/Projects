...
Original file is located at
    https://colab.research.google.com/drive/1RyfuiJU10Al9JwjgRzkCqGrixcF_HkBo
"""
...
def decode(input_length, tagset, score):
    ...
    v = [dict() for i in range(input_length)]
    b = [dict() for i in range(input_length)]
    for tag in tagset:
        v[1][tag] = score(tag,"<START>",1)
        b[1][tag] = "<START>"
    ...
    for i in range(2,input_length-1):
        for tag in tagset:
            ...
            max_score = None
            for t in tagset:
                if max_score is None or max_score < (v[i-1][t]+score(tag,t,i)):
                    max_score = v[i-1][t]+score(tag,t,i)
                    max_tag = t
    ...
    for t in tagset:
        b[-1][t] = t
        v[-1][t] = v[-2][t]+score("<STOP>",t,input_length-1)
    
    best_path_pointer = max(v[-1], key=v[-1].get)
    tag_seq = [best_path_pointer,"<STOP>"]
    for i in range(input_length-2,0,-1):
        tg = tag_seq[0]
        tag_seq.insert(0,b[i][tg])
